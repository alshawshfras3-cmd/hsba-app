...
// اختبارات إلزامية ومصححة
try {
  // اختبار 1: الراجحي — شركات (basic_housing، divisor 480)
  // approvedSalary = 9,000+3,000 = 12,000، serviceMonths = 298
  // pension = round(12000 × 298 / 480) = round(7,450) = 7,450
  const test1 = calculatePensionFromRule({
    approvedSalary: 12000, serviceMonths: 298, yearsToRetirement: 10,
    rule: { id: 't1', bankId: 'rajhi', sectorId: 'companies', calculationMethod: 'service_based', divisorMonths: 480 }
  });
  console.assert(
    test1.pension === 7450,
    `❌ الراجحي شركات خاطئ: المتوقع 7450 وجد ${test1.pension}`
  );

  // اختبار 2: الراجحي — مدني (basic × 1.345، divisor 480)
  // approvedSalary = 9,000 × 1.345 = 12,105، serviceMonths = 298
  // pension = round(12105 × 298 / 480) = round(7,515.3) = 7,515
  const test2 = calculatePensionFromRule({
    approvedSalary: 12105, serviceMonths: 298, yearsToRetirement: 10,
    rule: { id: 't2', bankId: 'rajhi', sectorId: 'gov_civil', calculationMethod: 'service_based', divisorMonths: 480 }
  });
  console.assert(
    test2.pension === 7515,
    `❌ الراجحي مدني خاطئ: المتوقع 7515 وجد ${test2.pension}`
  );

  // اختبار 3: الأهلي — قطاع قوي، 3 سنوات للتقاعد (≤5) → 70%
  // approvedSalary = 10,000، rate = 70%
  // pension = 7000
  const test3 = calculatePensionFromRule({
    approvedSalary: 10000, serviceMonths: 300, yearsToRetirement: 3,
    rule: { id: 't3', bankId: 'ahli', sectorId: 'strong', calculationMethod: 'fixed_percentage',
            rateBelowThreshold: 70, rateAboveThreshold: 80, yearsThreshold: 5 }
  });
  console.assert(
    test3.pension === 7000,
    `❌ الأهلي قوي ≤5 سنوات خاطئ: المتوقع 7000 وجد ${test3.pension}`
  );

  // اختبار 4: الأهلي — قطاع قوي، 8 سنوات للتقاعد (>5) → 80%
  // approvedSalary = 10,000، rate = 80%
  // pension = 8000
  const test4 = calculatePensionFromRule({
    approvedSalary: 10000, serviceMonths: 300, yearsToRetirement: 8,
    rule: { id: 't4', bankId: 'ahli', sectorId: 'strong', calculationMethod: 'fixed_percentage',
            rateBelowThreshold: 70, rateAboveThreshold: 80, yearsThreshold: 5 }
  });
  console.assert(
    test4.pension === 8000,
    `❌ الأهلي قوي >5 سنوات خاطئ: المتوقع 8000 وجد ${test4.pension}`
  );
} catch (e) {
  console.error("error in assertions:", e);
}
...
// === AUTOMATIC ENGINE VERIFICATION TEST SUITE ===
try {
  console.log("=== بدء تشغيل اختبارات الراتب التقاعدي الموحد الجديدة ===");

  // اختبار 1: الراجحي — مدني — 4 سنوات متبقية
  const test1 = calculatePensionFromTemplate({
    template: defaultLibraryRules.find(t => t.id === 'tpl_rajhi_civil')!,
    basicSalary: 9000,
    housingAllowance: 2250,
    otherAllowances: 1500,
    netSalary: 12000,
    currentServiceMonths: 300,
    monthsToRetirement: 48
  });
  console.assert(test1 === 6525, `❌ فشل اختبار 1: المتوقع 6525 وجد ${test1}`);

  // اختبار 2: الراجحي — عسكري — 10 سنوات متبقية
  const test2 = calculatePensionFromTemplate({
    template: defaultLibraryRules.find(t => t.id === 'tpl_rajhi_military')!,
    basicSalary: 9000,
    housingAllowance: 2250,
    otherAllowances: 1500,
    netSalary: 12000,
    currentServiceMonths: 240,
    monthsToRetirement: 120
  });
  const expectedTest2 = (30 * (9000 * Math.pow(1.025, 10))) / 35;
  console.assert(Math.abs(test2 - expectedTest2) < 0.05, `❌ فشل اختبار 2: المتوقع ${expectedTest2} وجد ${test2}`);

  // اختبار 3: الأهلي — مدني — 7 سنوات متبقية
  const test3 = calculatePensionFromTemplate({
    template: defaultLibraryRules.find(t => t.id === 'tpl_fixed_strong')!,
    basicSalary: 9000,
    housingAllowance: 2250,
    otherAllowances: 1500,
    netSalary: 12750,
    currentServiceMonths: 300,
    monthsToRetirement: 84
  });
  console.assert(test3 === 9000, `❌ فشل اختبار 3: المتوقع 9000 وجد ${test3}`);

  // اختبار 4: الأهلي — خاص — 3 سنوات متبقية
  const test4 = calculatePensionFromTemplate({
    template: defaultLibraryRules.find(t => t.id === 'tpl_fixed_weak')!,
    basicSalary: 8000,
  ...
  console.assert(test4 === 6000, `❌ فشل اختبار 4: المتوقع 6000 وجد ${test4}`);

  // اختبار 5: الراجحي — القطاع الخاص — مصدر الراتب
  const test5 = calculatePensionFromTemplate({
    template: defaultLibraryRules.find(t => t.id === 'tpl_rajhi_companies')!,
    basicSalary: 9000,
    housingAllowance: 2250,
    otherAllowances: 1500,
    netSalary: 12750,
  ...
  console.assert(test6 === 6468.75, `❌ فشل اختبار 6: المتوقع 6468.75 وجد ${test6}`);

  console.log("✅ نجحت جميع اختبارات الراتب التقاعدي الموحد الستة الجديدة بنسبة 100%!");
} catch (testError) {
  console.error("❌ حدث خطأ أثناء تنفيذ اختبارات التقاعد الجديدة:", testError);
}
